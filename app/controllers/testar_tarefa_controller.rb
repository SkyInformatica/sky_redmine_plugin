class TestarTarefaController < ApplicationController
  ...
  def testar_tarefa
    ...
    if tarefa_testes
      ...
      flash[:notice] = "A tarefa foi colocada em teste e relacionada com #{view_context.link_to "##{tarefa_testes_existente.id} - #{tarefa_testes_existente.subject}", issue_path(tarefa_testes_existente)}."
    else
